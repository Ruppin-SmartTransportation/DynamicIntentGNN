...
| 21-23 | `current_edge_num_lanes_oh` | one-hot encoding of current edge's num_lanes (1, 2, or 3 lanes); [0,0,0] for junctions |
| 24    | `current_edge_demand`     | Demand value for the current edge (from updated edge features) |
| 25    | `current_edge_occupancy`  | Occupancy value for the current edge (from updated edge features) |
...
class DatasetCreator:
    ...
    def process_junctions(self):
        ...
        for jid in junction_ids:    
            features = [0] * NODE_FEATURES_COUNT  # Initialize with zeros for all 26 features
    ...
    def process_vehicle_features(self, snap_file, current_vehicle_ids, static_edge_ids_to_index, static_edge_features):
        ...
        | 18    | `j_type`            | junction type, "priority", "traffic_light"              |
        | 19    | `destination_x`     | normalized destination x coordinate                      |
        | 20    | `destination_y`     | normalized destination y coordinate                      |
        | 21-23 | `current_edge_num_lanes_oh` | one-hot encoding of current edge's num_lanes (1, 2, or 3 lanes); [0,0,0] for junctions |
        | 24    | `current_edge_demand`     | Demand value for the current edge (from updated edge features) |
        | 25    | `current_edge_occupancy`  | Occupancy value for the current edge (from updated edge features) |
    ...
    def create_dataset(self):
        ...
        expected_junction_features = NODE_FEATURES_COUNT - 1  # Exclude j_type
        ...
        for snap_file in tqdm(self.snapshot_files, desc="Processing snapshots"):
            ...
            for i, features in enumerate(vehicle_features):
                if len(features) != NODE_FEATURES_COUNT:
                    print(f"ERROR: Vehicle {i} has {len(features)} features, expected {NODE_FEATURES_COUNT}. Terminating.")
                    exit(1)
